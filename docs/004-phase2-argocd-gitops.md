...
### 검증 스크립트 자체의 경쟁 조건 — 수정함

머지 후 `dev` 기준으로 재검증할 때 조건 5가 실패했다. 파드가 11개가 아니라 13개로 잡혔다.

```
catalog-api-8856f994d-9746r   0/1  Terminating  14s
catalog-api-8856f994d-rbptm   0/1  Terminating  14s
```

클러스터 문제가 아니라 검증 스크립트가 만든 상황이었다.
조건 2(selfHeal)가 복제본을 일부러 5개로 늘렸다가 되돌리는데,
그때 생긴 파드가 정리되기 전에 조건 4·5가 개수를 세기 시작했다.

ArgoCD 는 `spec.replicas` 를 4초 만에 되돌리지만 파드의 실제 종료는 그보다 오래 걸린다.
`deletionTimestamp` 가 붙은 파드가 사라질 때까지 기다리는 단계를 조건 4 앞에 넣었다.

검증이 스스로 만든 조건 때문에 실패한 사례다.
클러스터를 망가뜨려 보는 검증은 다음 검사를 시작하기 전에 원상 복구를 확인해야 한다.
