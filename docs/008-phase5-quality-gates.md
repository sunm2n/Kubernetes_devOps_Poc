...
### 4. 주문 검증이 페이지 경계에서 깨졌다 — 수정함

머지 후 재검증에서 조건 6이 실패했다.

```
✗ FAIL  30초 내에 주문이 생성되지 않았다
```

파드는 11개 모두 Running·재시작 0회였고 Ordering 로그도 정상이었다.
확인해보니 **주문은 만들어져 있었다.**

```
count: 11   pageIndex: 0   pageSize: 10
1페이지: ORD_1, ORD_2, poc-1786611544, ... poc-1786669644
2페이지: poc-1786669847     ← 방금 만든 주문
```

`scripts/12-verify-eshop.sh` 가 목록(`/orders`)을 받아 사용자명이 있는지 보고 있었다.
목록은 페이지 크기 10으로 잘려 나오고, 검증을 반복할수록 주문이 쌓인다.
**11번째부터 새 주문이 1페이지에서 사라지면서** 이벤트 흐름은 정상인데 검증만 실패했다.

이름으로 직접 조회하도록 고쳤다(`/orders/{orderName}`).
이 엔드포인트는 없는 이름에도 HTTP 200 을 주므로 본문으로 판단한다.

```
있음: {"orders":[{...}]}      없음: {"orders":[]}
```

Phase 1부터 있던 결함인데 데이터가 쌓이기 전까지는 드러나지 않았다.
**검증이 우연히 통과하고 있었던 것**이고, 실패했을 때 원인이 애플리케이션이 아니라
검증 쪽인 경우를 다시 확인한 사례다.
